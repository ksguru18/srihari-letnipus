    ...
    public void create(UserLoginPassword item) {
        ...
        try (LoginDAO loginDAO = MyJdbi.authz()) {
            ...
            if (obj == null) {
                ...
                if (item.getPasswordHash() == null || item.getSalt() == null || item.getAlgorithm() == null) {
                    throw new WebApplicationException("Password hash, salt and algorithm values  must be specified during user login password creation", Response.Status.BAD_REQUEST);
                }
