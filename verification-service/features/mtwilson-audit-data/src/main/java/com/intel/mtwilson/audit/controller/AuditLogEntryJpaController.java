...
public class AuditLogEntryJpaController implements Serializable {
    ...
    public void rotate(int maxRowCount, int numRotations) {
        EntityManager em = getEntityManager();
        try {
            em.getTransaction().begin();
            StoredProcedureQuery query = em
                .createStoredProcedureQuery("rotate_audit_log_partitions")
                .registerStoredProcedureParameter("max_row_count", 
                    Integer.class, ParameterMode.IN)
                .registerStoredProcedureParameter("num_rotations", 
                    Integer.class, ParameterMode.IN)
                .setParameter("max_row_count", maxRowCount)
                .setParameter("num_rotations", numRotations);
            query.execute();
            em.getTransaction().commit();
        } finally {
                em.close();
    ...
    public AuditLogEntry findAuditLogEntry(String id) {
        EntityManager em = getEntityManager();
        try {
